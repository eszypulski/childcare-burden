...
const y = d3.scaleLinear().range([height - 10, 0]);
...
d3.csv("datasets/ccdata.csv").then(data => {
    ...
    let filteredData;
    let buttonData = 'mcinfant'; // Initial data source
    ...
    const area = d3.area().x(d => x(d.date)).y0(height - 10);
    ...
    svg.append("path")
        .attr("class", "area mhi")
        .style("fill", "#DADADA") // CHANGE THE COLOR LATER
        .style("opacity", .9); // CHANGE THE OPACITY LATER

    // ADD THE LINE PATH TO THE CHART
    svg.append("path")
        .attr("class", "line mhi")
        .style("fill", "none")
        .style("stroke", "#DADADA") // CHANGE THE COLOR LATER
        .style("stroke-width", 2); // CHANGE THE WIDTH LATER

    // SET UP THE INFANT CARE LINE GENERATOR
    const lineInfant = d3.line().x(d => x(d.date));

    // SET UP THE AREA GENERATOR
    const areaInfant = d3.area().x(d => x(d.date)).y0(height - 10);

    // ADD THE AREA PATH TO THE CHART
    svg.append("path")
        .attr("class", "area mcinfant")
        .style("fill", "#21897E") // CHANGE THE COLOR LATER
        .style("opacity", .9); // CHANGE THE OPACITY LATER
    ...
    svg.append("path")
        ...
        .style("fill", "none")
        .style("stroke", "#21897E") // CHANGE THE COLOR LATER
        .style("stroke-width", 2); // CHANGE THE WIDTH LATER 

    // Function to update the chart based on the selected county and data source
    function updateChart() {
        ...
        // Use the selected data source for the infant care line and area
        svg.select(".area.mcinfant")
            .datum(filteredData)
            .attr("d", areaInfant.y1(d => y(d[buttonData])));

        svg.select(".line.mcinfant")
            .datum(filteredData)
            .attr("d", lineInfant.y(d => y(d[buttonData])));
    }
    ...
    // Add event listeners to the buttons to toggle the data source and update the chart
    d3.select("#buttonBenchmark").on("click", function () {
        buttonData = 'benchmark';
        updateChart();
    });

    d3.select("#buttonActual").on("click", function () {
        buttonData = 'mcinfant';
        updateChart();
    });
});
